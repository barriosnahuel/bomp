...
- **Changing the app startup path** (Application/Activity `onCreate`, first-frame Composables, startup-time deps): regenerate the committed Baseline Profile → `./scripts/generate-baseline-profile.sh` (procedure + when in CONTRIBUTING.md § *Baseline Profile*). Stale = slower first launches, never broken.
