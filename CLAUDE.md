...
Audit case-by-case, never blind-delete: a long block that is *local justification* with no ADR home (a multi-key contract, the `StrictModeConfigurator` matchers) stays. A single comment block over **25 lines** is grep-flagged by `scripts/check-adr-invariants.sh`; trim it to a pointer, or acknowledge a genuinely-legit long block with a `long-comment-ok` marker inside it (same pattern as `// alpha-ok` / `// button-ok`).
