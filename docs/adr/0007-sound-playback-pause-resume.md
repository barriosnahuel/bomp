...
| `pause()` | No-op for `Sound`; only meaningful for `Uri`. | Pauses the current target (Sound or Uri). For Sound: saves `currentPosition` in `savedSoundPositions[t.sound.name]` and fires `onPlayerStop(t.sound, completed=false)`. For Uri: updates `_playbackState.copy(isPlaying=false, positionMs=...)`. |
