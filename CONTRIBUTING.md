...
It runs as a **Claude Code Desktop Scheduled Task** (Fridays 18:00 GMT-3). To keep the source in the repo, the task is wired as a *thin pointer*:

1. Grant the scheduled task **project access** to this repo directory (so it can resolve the committed skill), and keep the "Google Cloud BigQuery" connector attached (it provides the query tools + auth — no `.mcp.json` or service-account is needed).
2. Set the task's Instructions to a one-liner: *"Ejecutá el skill `/exec-report` y entregá el resultado en esta conversación."*
