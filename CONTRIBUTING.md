...
The cold-start hot path is AOT-compiled from `app/src/main/baseline-prof.txt`, which AGP bakes into the release/`benchmark` APK (`assets/dexopt/baseline.prof`). We **don't** use the `androidx.baselineprofile` plugin: it would add `nonMinifiedRelease`/`benchmarkRelease` app variants that each need their own `google-services.json`. Instead the profile is generated by hand and committed; regenerate it when the startup path changes meaningfully.

Rules must carry **real (non-obfuscated) names** so AGP can remap them through R8 at release time, so generate against the **non-minified `debug`** build (the minified `benchmark` build would bake obfuscated names that don't match a fresh release):

```bash
SERIAL=<your-device>          # a real device; emulators can't measure AOT (inverted numbers)
ANDROID_SERIAL=$SERIAL ./gradlew :app:installDebug :macrobenchmark:assembleBenchmark
adb -s $SERIAL install -r macrobenchmark/build/outputs/apk/benchmark/macrobenchmark-benchmark.apk
adb -s $SERIAL shell am instrument -w \
  -e class com.github.barriosnahuel.vossosunboton.macrobenchmark.BaselineProfileGenerator \
  com.github.barriosnahuel.vossosunboton.macrobenchmark/androidx.test.runner.AndroidJUnitRunner
adb -s $SERIAL pull "/storage/emulated/0/Android/media/com.github.barriosnahuel.vossosunboton.macrobenchmark/BaselineProfileGenerator_generate-baseline-prof.txt" app/src/main/baseline-prof.txt
```

Validate on a **real device** (the emulator's compilation numbers are non-representative): `StartupBenchmark.startupBaselineProfile` (`CompilationMode.Partial(Require)`) should land near `startupDefaultCompilation` and well under `startupNoCompilation`.
