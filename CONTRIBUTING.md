...
- **`HARD_CAP_SECONDS`** (default `2400`) — total wall clock for one run, whatever it's doing. Backstop for a hang that keeps dribbling output. 2× the slowest complete run measured (1186 s), and it discounts an observed host sleep like the stall clock does, so a suspend doesn't cap a healthy run.
