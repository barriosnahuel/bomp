...
- **`STALL_TIMEOUT_SECONDS`** (default `420`) — **test phase only**, i.e. after the runner announces `run started:`. From here TestRunner narrates every test, so silence really does mean the guest stopped executing. Floored at the runner's on-device `timeout_msec` (read from `app/build.gradle`): a value at or below it is raised automatically, because a deadlocked test is silent until that fires and names it — a tighter clock would kill the run blind, as a stall, right when the runner was about to report the deadlock as a real red.
...
- **`HARD_CAP_SECONDS`** (default `2700`) — total wall clock for one run, whatever it's doing. Backstop for a hang that keeps dribbling output; the stall clock is the primary detector, so this stays deliberately generous (2.3× the slowest complete run measured, 1186 s) — erring low would false-abort a healthy cold-cache run, which the calibration runs (warm-cache, same-commit) never exercised. It discounts an observed host sleep like the stall clock does, so a suspend doesn't cap a healthy run.

The defaults come from 8 measured runs (ADR 0001 § *Bounded termination*), and `STALL_TIMEOUT_SECONDS` is **not** sized against the slowest test (35 s) — it's sized against `timeout_msec` (300 s, `app/build.gradle`). A test deadlocked on-device goes completely silent until *its own* timeout fires and names it, so a stall clock under 300 s kills the run blind exactly when the runner was about to tell you which test hung. If you raise `timeout_msec`, raise this with it; `flaky-report.sh` warns when the two cross.
...
- **⚠ truncated runs** — a run that recorded fewer tests than the runner announced. The instrumentation process died under a test (a StrictMode kill, a native crash) and everything after it never ran. It exits `1` like any red, so nothing else tells you that a "single failure" also skipped the other 110 tests — and its silence about them is not evidence they pass.
...
- **slowest tests (p95)** — the headroom left on `STALL_TIMEOUT_SECONDS`, counting only tests that *finished*. A red's `time=` is the clock that ran out, not work done: a test killed at `timeout_msec` reports a flat 300 s, and letting that count as "the slowest test" would calibrate the timeout against its own failure mode. The report also warns if the stall clock ever drops below `timeout_msec`.
...
- [ ] **Finalize `CHANGELOG.md`** — stamp `## [unreleased]` as `## [vYYYY.MM.N] - YYYY-MM-DD`, the cut month + the month's next counter (`.1` for the month's first release), keeping the Keep-a-Changelog cut-date suffix (§ CLAUDE.md *Changelog*). The tag must match the open milestone — if it was carried over from an earlier no-release month, rename it first to the cut month plus the month's next counter (§ *Creating the GitHub release*, step 6).
...
6. **Roll the milestone — create, sweep, close, in that order** ([ADR 0029](docs/adr/0029-one-open-milestone-invariant.md)); exactly one milestone is open at every instant, and every PR takes that one (CLAUDE.md § *Labels and milestone*). Commands in § *Labels & milestone examples → Milestone operations*.

   1. **Create** `vYYYY.MM.<N+1>` — the cut month, counter +1 (cutting `v2026.09.1` creates `v2026.09.2`). **Before** closing, so no instant passes with nothing open to assign.
   2. **Sweep** every still-open PR off the closing milestone onto the new one — a PR in flight at the cut did **not** ship in this release, and GitHub's only signal is a closed milestone with `open_issues > 0`, which nobody watches.
   3. **Close** the release's milestone, now empty of open PRs.

   Deferring the create until the next PR opens is what files a post-cut PR under the release that already shipped — until the new milestone exists there is nothing correct to assign. If the month then ends with no second release, rename the open milestone to the next month (`v2026.09.2` → `v2026.10.1`, § *Versioning*) — the rename absorbs the counter guess.
...
**Co-evolution with the weekly report.** Adding, renaming, or removing an
analytics event/param/track → update `.claude/skills/exec-report/SKILL.md` in
the **same PR** (kin to the [Third-party notices](#third-party-notices-) rule).
That skill is the versioned source of truth for the weekly executive report
(§ [BigQuery export](#bigquery-export-)); if the instrumentation moves and the
skill doesn't, the report silently goes blind to the new event.
...
### Weekly executive report (versioned source of truth)

The weekly Bomp executive report (business / product-funnel / quality / growth / performance, delivered in Spanish) is generated by the committed **`/exec-report`** skill (`.claude/skills/exec-report/`) — that file is the single source of truth for its queries, methodology, and output format. Edit the report by editing the skill (PR-reviewable), not by re-pasting a prompt.

It runs as a **Claude Code Desktop Scheduled Task** (Fridays 18:00 GMT-3). The skill is versioned, but **the task trigger that fires it is not** — it lives in the Claude app's own config, so on a clean machine ("setup de 0") it has to be rebuilt. The full from-scratch rebuild, no prior context assumed:

**Prerequisites**

- Claude Code (Desktop) installed and signed in.
- This repo cloned, with the working dir checked out to a branch that has `.claude/skills/exec-report/` (i.e. `develop` once this lands).
- The **"Google Cloud BigQuery" connector** (added from the Claude app's connector settings) authenticated with read access to `bomp-prod` — it provides the query tools + auth, so no `.mcp.json` or service-account is needed.
...
The report's Performance/Quality axes apply the same anti-false-alarm methodology as `/perf-report-triage` (median + per-device + min-N guard) so a single bad device can't manufacture a release regression — when the report flags a perf regression, run `/perf-report-triage` to confirm it's same-device before treating it as a code bug.
...
The *rule* (one type label + optional concern labels; the one **open** milestone assigned at PR creation) and the bare label names live in CLAUDE.md § *Labels and milestone*. Full "when to use" per label, then worked combinations:

### Milestone operations

**A PR never creates or derives a milestone** — it takes the one that is open ([ADR 0029](docs/adr/0029-one-open-milestone-invariant.md)). Creating is the cut's job (§ *Creating the GitHub release*, step 6), which names the new one `vYYYY.MM.<N+1>` from the cut month. **Deriving `N` from scratch** is only needed when nothing is open at all (project start, or a milestone closed out of band): take the next counter after the month's last cut — check existing `vYYYY.MM.*` tags and closed milestones — and `.1` only if the month has not cut anything yet. `gh` has no first-class milestone commands, so use the API:

```bash
gh api repos/{owner}/{repo}/milestones --jq '.[] | "\(.number) \(.title)"'   # the open one: what a PR takes
gh api repos/{owner}/{repo}/milestones -f title="v2026.09.2"                   # create (at cut, before closing)
gh pr list --search 'milestone:"v2026.09.1" is:open' --json number             # sweep: PRs in flight at the cut
gh api -X PATCH repos/{owner}/{repo}/milestones/<number> -f state=closed        # close (after the sweep)
gh api -X PATCH repos/{owner}/{repo}/milestones/<number> -f title="v2026.10.1" # rename on a no-release month
gh pr create --milestone "v2026.09.2" ...                                      # assign at PR creation
```
