...
Rules must carry **real (non-obfuscated) names** so AGP can remap them through R8 at release time, so generation runs against the **non-minified `debug`** build (the minified `benchmark` build would bake obfuscated names that don't match a fresh release). One command does it — install debug, run the `BaselineProfileGenerator`, pull into `app/src/main/baseline-prof.txt`:
