...
- A committed Baseline Profile (`app/src/main/baseline-prof.txt`) AOT-compiling the LandingActivity cold-start path, baked into the release APK by AGP. Generated by hand against the non-minified debug build via a `BaselineProfileGenerator` (no `androidx.baselineprofile` plugin — it would add app variants each needing their own `google-services.json`); `StartupBenchmark` gained a `Partial(Require)` mode that validates the profile lands near the warmed `DEFAULT` baseline and far under no-AOT `None`
