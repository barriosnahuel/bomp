...
- Instrumented runs now record themselves to a local history (`.instrumented-history/` in the primary worktree, gitignored), and `scripts/flaky-report.sh` reads it to answer the question a red actually raises — is this a known flaky, or did I just break it? It separates a test that fails *some* runs of the same commit (🟡 flaky, re-running is legitimate) from one that fails *every* run (🔴 consistent, re-running will not help), groups stalls by where they died and whether the host was asleep, and reports per-test p95 so the stall timeout's headroom stays visible as tests are added
...
#### Changed
- `scripts/run-instrumented-tests.sh` now supervises the run under a stall watchdog, so a wedged emulator ends the run instead of hanging forever unnoticed: it prints a live per-test counter parsed from AndroidJUnitRunner's logcat narration, kills the run when test progress stops (a separate, larger budget covers the legitimately-silent build phase, so a slow build is never mistaken for a hang), dumps a diagnostics bundle probing whether the *emulator* froze or *Gradle/ddmlib* got stuck, and reports the outcome as an exit code where only `1` means "a test is genuinely red" — `124` means "the emulator hung, re-run" and `3` means "the build never reached the tests". Behaviour-tested over stubbed adb/emulator/gradlew, asserting the healthy paths as hard as the stall ones (CI job `instrumented-watchdog-test`); AGP's `installation.timeOutInMs` + the runner's `timeout_msec` are kept as scoped defence in depth for direct Gradle invocations (ADR 0001 § *Bounded termination*)
